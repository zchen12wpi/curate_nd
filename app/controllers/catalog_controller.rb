  ...
  configure_blacklight do |config|
    ...
    config.add_search_field('all_fields', label: 'All Fields', :include_in_advanced_search => false) do |field|
      ...
      field.solr_parameters = {
        :qf => "#{title_name} #{label_name} noid_tsi file_format_tesim #{contributor_name} #{abstract_name} #{description_name} #{creator_name} #{administrative_unit} #{publisher_name} #{language_name} #{collection_name} #{contributor_institution_name} #{subject_name} #{identifier_name} #{urn} #{degree_name} #{degree_disciplines} #{contributors} #{degree_department_acronyms} #{date_created} #{source}",
        :pf => "#{title_name}"
      }
    end
